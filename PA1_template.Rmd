...
The mean total number of steps taken per day is `r round(mean(activitySum$dailySteps), digits=2)`.  
The median total number of steps taken per day is `r median(activitySum$dailySteps)`.
...
number of steps is the interval `r subset(activityPattern$interval, 
       activityPattern$avgSteps==max(activityPattern$avgSteps))`, or 8:35-8:40AM. 
...
```
There are now `r sum(is.na(activityNoNa$steps))` missing values.   
...
`r as.character(round(mean(activitySum2$dailySteps)))` because the imputation process used here sets 
...
`r as.character(round(mean(activitySum2$dailySteps)))` becomes the median as well.
...
Activity is slightly higher on weekends: the mean steps per interval on weekdays is `r round(mean(weekdays), 2)` and the mean steps per interval on weekends is greater at `r round(mean(weekends),2)`. Additionally, there is a more pronounced activity spike between the 800 and 900 intervals of weekday days, corresponding to 8:00-9:00 AM, compared to weekend days,

...
```  
The mean steps per interval on weekdays is `r round(mean(weekdays), 2)` and the
mean steps per interval on weekends is slightly greater at `r round(mean(weekends),2)`.
